...
fn fund_account_returns_error_on_mainnet() {
    with_network("mainnet", || {
        ...
        assert!(
            matches!(result.unwrap_err(), StellarError::FriendbotNotAvailable { .. }),
...
fn friendbot_not_available_display_mentions_network_and_alternatives() {
    ...
    assert!(msg.contains("Mainnet"), "Error message should contain the network name");
    assert!(
...
fn testnet_config_exposes_friendbot_url() {
    ...
    assert!(cfg.friendbot_url.is_some(), "Testnet config must have a Friendbot URL");
...
fn mainnet_config_has_no_friendbot_url() {
    ...
    assert!(cfg.friendbot_url.is_none(), "Mainnet config must NOT have a Friendbot URL");
}

#[test]
fn futurenet_config_exposes_friendbot_url() {
    let cfg = NetworkConfig::futurenet();
    assert!(cfg.friendbot_url.is_some(), "Futurenet config must have a Friendbot URL");
}
