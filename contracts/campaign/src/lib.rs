...
impl CampaignContract {
    ...
    pub fn create(env: Env, campaign_id: Symbol, title: Symbol, target: i128, deadline: u64) {
        ...
        env.storage().instance().set(&key, &(campaign_id, title, target, deadline));
    }
    ...
    pub fn is_active(env: Env) -> bool {
        let key = Symbol::new(&env, "campaign_data");
        let (_id, _title, _target, deadline): (Symbol, Symbol, i128, u64) = env.storage().instance().get(&key).unwrap();
