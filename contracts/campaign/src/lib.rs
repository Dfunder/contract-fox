...
use soroban_sdk::{
    Address, Env, Symbol, contract, contractimpl, 
    Map, Vec, symbol_short
};
...
impl CampaignContract {
    ...
    pub fn register_campaign(env: Env, owner: Address, goal: i128, deadline: u64) -> u64 {
        ...
        let mut campaigns: Map<u64, Campaign> = env.storage().instance().get(&CAMPAIGN_MAP).unwrap_or(Map::new(&env));
    ...
    pub fn get_campaign(env: Env, campaign_id: u64) -> Campaign {
        let campaigns: Map<u64, Campaign> = env.storage().instance().get(&CAMPAIGN_MAP)
    ...
    pub fn update_campaign_status(env: Env, campaign_id: u64, status: u32) {
        let mut campaigns: Map<u64, Campaign> = env.storage().instance().get(&CAMPAIGN_MAP)
            .unwrap_or_else(|| panic!("No campaigns found"));
