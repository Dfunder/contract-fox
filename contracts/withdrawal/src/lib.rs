...
impl WithdrawalContract {
    ...
    pub fn initialize(env: Env, beneficiary: Address, max_withdrawal: i128) {
        ...
        env.storage().instance().set(&key, &(beneficiary, max_withdrawal));
    }

    /// Withdraw funds from the contract
    pub fn withdraw(env: Env, amount: i128) -> bool {
        let key = Symbol::new(&env, "settings");
        let (beneficiary, max_withdrawal): (Address, i128) = env.storage().instance().get(&key).unwrap();
        ...
        env.storage().instance().set(&withdrawn_key, &(total + amount));
